...
class DIDD(nn.Module):
    def __init__(self, sep, input_size):

        size = input_size // 64 # Final output size after encoding
        self.E_common = E_common(sep, size)
        self.E_dx = E_separate_A(sep, size)
        self.E_ctl = E_separate_B(sep, size)
        self.decoder = Decoder(size)
        self.discriminator = Disc(sep, size)
...
class E_common(nn.Module):
    def __init__(self, sep, size, total_out_channels=512):
        ...
        self.layer5 = []
        self.layer6 = []
        self.z_dim_size = (self.dim - 2 * self.sep) * np.product(self.size)

        self.layer1.append(SpectralNorm(nn.Conv3d(3, 32, 4, 2, 1)))
        self.layer1.append(nn.InstanceNorm3d(32))
        ...
        self.layer1.append(nn.InstanceNorm3d(64))
        ...
        self.layer3.append(nn.InstanceNorm3d(128))
        ...
        self.layer4.append(nn.InstanceNorm3d(256))
        self.layer4.append(nn.LeakyReLU(0.2, inplace=True))
        self.l4 = nn.Sequential(*self.layer4)

        self.layer5.append(SpectralNorm(nn.Conv3d(256, (self.total_out_channels - self.sep), 4, 2, 1)))
        self.layer5.append(nn.InstanceNorm3d(self.total_out_channels - self.sep))
        self.layer5.append(nn.LeakyReLU(0.2, inplace=True))
        self.l5 = nn.Sequential(*self.layer5)

        self.layer6.append(SpectralNorm(nn.Conv3d((self.total_out_channels - self.sep), (self.total_out_channels -
                                                                                         2 * self.sep), 4, 2, 1)))
        self.layer6.append(nn.InstanceNorm3d(total_out_channels - 2 * self.sep))
...
class E_separate_A(nn.Module):
    def __init__(self, sep, size):
        ...
        self.layer2.append(SpectralNorm(nn.Conv3d(32, 64, 4, 2, 1)))
        self.layer2.append(nn.InstanceNorm3d(64))
        self.layer2.append(nn.LeakyReLU(0.2, inplace=True))
        self.l2 = nn.Sequential(*self.layer2)
        ...
        self.layer5.append(SpectralNorm(nn.Conv3d(256, 512, 4, 2, 1)))
        self.layer5.append(nn.InstanceNorm3d(512))
        self.layer5.append(nn.LeakyReLU(0.2, inplace=True))
        self.l5 = nn.Sequential(*self.layer5)

        self.layer6.append(SpectralNorm(nn.Conv3d(512, self.sep, 4, 2, 1)))
        self.layer6.append(nn.InstanceNorm3d(self.sep))
        self.layer6.append(nn.LeakyReLU(0.2, inplace=True))
        self.l6 = nn.Sequential(*self.layer6)
    ...
    def forward(self, net):
        ...
        out = self.l5(out)
        out = self.l6(out)
        out = out.view(-1, self.sep * np.product(self.size))
        return out
...
class E_separate_B(nn.Module):
    def __init__(self, sep, size):
        ...
        self.layer5.append(SpectralNorm(nn.Conv3d(256, 512, 4, 2, 1)))
        self.layer5.append(nn.InstanceNorm3d(self.sep))
        self.layer5.append(nn.LeakyReLU(0.2, inplace=True))
        self.l5 = nn.Sequential(*self.layer5)

        self.layer6.append(SpectralNorm(nn.Conv3d(512, self.sep, 4, 2, 1)))
        self.layer6.append(nn.InstanceNorm3d(512))
        self.layer6.append(nn.LeakyReLU(0.2, inplace=True))
        self.l6 = nn.Sequential(*self.layer6)
